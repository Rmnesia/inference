      ...
      <TabContext value={value}>
        <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
          <TabList value={value} onChange={handleTabChange} aria-label="tabs">
            <Tab label="语言模型" value="/train_model/custom/llm" />
            <Tab
              label="嵌入模型"
              value="/train_model/custom/embedding"
            />
            <Tab label="重排模型" value="/train_model/custom/rerank" />
            <Tab label="图像模型" value="/train_model/custom/image" />
            <Tab label="音频模型" value="/train_model/custom/audio" />
          </TabList>
        </Box>
        {customType.map((item) => (
          <TabPanel
            key={item}
            value={`/train_model/custom/${item}`}
            sx={{ padding: 0 }}
          >
            <div
              style={{
